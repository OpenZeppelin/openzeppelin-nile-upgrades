...
async def declare_contract(network, contract_name, account, max_fee, overriding_path=None):
    """
    Declare a contract and waits until the transaction completes.

    Returns the declared class hash in decimal format.
    """
    logging.debug(f"Declaring contract {contract_name}...")
    ...
    if class_hash_exists(class_hash, network):
        logging.debug(f"Contract with hash {padded_hash} already exists")
    else:
        ...
        if tx_status.status.is_rejected:
            raise Exception(f"Could not declare contract. Transaction rejected.", tx_status.error_message)

        logging.debug(f"Contract declared with hash {padded_hash}")
